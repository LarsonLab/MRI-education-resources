% spectral-spatial RF pulse design

%%
clear imall
write_animations = 1;

GAMMA = 4257; % Hz/G

slewmax = 20e3; % G/cm/s
gmax = 4; % G/cm
T = 20e-3; % s
TBW = 3;
SBW = 6;
dz = 0.5; % cm, max spatial resolution
Nspec = 10;
DT = T/Nspec;
dt = 100e-6; % s
flip = pi/4;

%%
gamp = (SBW/DT)/(GAMMA*dz); % gradient for spatial resolution

rfspec = dzrf(Nspec, TBW);

verse_frac = 0.8;
Nlobe = DT/dt;
Nspat = round(Nlobe * verse_frac);
Nwait = (Nlobe - Nspat)/2;
rfspat = dzrf(Nspat, SBW);

% Uses a slew rate that can't be violated
% IE gamp = gmax would be ok (so can change slice thickness on console)
Nramp = min( ceil(gmax/slewmax / dt), floor(Nlobe/2));
ramp = [.5:Nramp-.5]/Nramp;
g1 = [ramp, ones(1,Nlobe-2*Nramp), ramp(end:-1:1)]; 
rfspatv = verse(g1, [zeros(1, floor(Nwait)), rfspat, zeros(1,ceil(Nwait))]).';
  
rf = []; g = [];
for k = 1:Nspec
   rf = [rf, rfspatv*rfspec(k)];
   g = [g, (mod(k,2)*2 - 1)*gamp*g1];
end

Nrw = round((Nlobe - Nramp)/2 + Nramp);
grw = [ramp, ones(1,Nrw-2*Nramp), ramp(end:-1:1)];

Nzeros = 20;

g = [g, (mod(Nspec+1,2)*2-1)*gamp*grw, zeros(1,Nzeros)];
rf = [rf, zeros(1,Nrw+Nzeros)];

flip_scale = flip/sum(rf);
rf = flip_scale*rf;
rfs = rfscaleg(rf, T*1e3);

Tall = [1:length(rf)]*dt;

%%

N = 25;

z = linspace(-0.8, 0.8, N);
gz = 2*pi*GAMMA*dt * g;

f = linspace(-500, 500, N);
gf = 2*pi*dt * ones(1,length(rf));

%mxy_2d = ab2ex(abr(rf, gz + i*gf, z, f));

[zplot, fplot] = meshgrid(z, f);
df = f(2)-f(1); dz = z(2)-z(1);
%%
for It = 1:length(rf)
    % SLR based simulation
    rftemp = rf(1:It);
    gsim = gz(1:It) + i*gf(1:It);
    mxy_2d =ab2ex(abr(rftemp, gsim, z, f));
    
% bloch_*

        fs = figure(1);
        subplot(122)
        IN = [1:N]-(N+1)/2;
        [INx INy] = meshgrid(IN,IN);
        quiver(INx-real(mxy_2d)/2,INy - imag(mxy_2d)/2,...
            real(mxy_2d), imag(mxy_2d),0)
        xlabel('f [Hz]'), ylabel('z [cm]')
        xlim([-N/2 N/2])
        ylim([-N/2 N/2])
        xticks([IN(1) IN(1)/2 0 IN(end)/2 IN(end)])
        xticklabels({num2str(f(1)),num2str(f(1)/2),'0', num2str(f(end)/2), num2str(f(end))}) 
        yticks([IN(1) IN(1)/2 0 IN(end)/2 IN(end)])
        yticklabels({num2str(z(1)),num2str(z(1)/2),'0', num2str(z(end)/2), num2str(z(end))}) 
        
        title('M_{XY}')
        
        
        subplot(221), plot(Tall*1e3, rfs*100, 1e3*Tall(It)*ones(1,2), [-1 4]), ylabel('RF [uT]'), xlim([0 Tall(end)*1e3])
        subplot(223), plot(Tall*1e3, g*10, 1e3*Tall(It)*ones(1,2), [-15 15]), ylabel('G_Z [mT/m]'), xlim([0 Tall(end)*1e3])
%        subplot(325), plot(Tall(1:It), real(measured_signal(1:It))), 
xlabel('time [ms]'), %ylabel('Signal')
%        ylim([-1 1]), xlim([0 3.5])
        

        drawnow
        
        currFrame = getframe(fs);
        if It == 1
            im = frame2im(currFrame);
            [imind,map] = rgb2ind(im,256,'nodither'); 
        end
        imall(:,:,1,It) = rgb2ind(frame2im(currFrame),map,'nodither');
%        Ix = Ix+1;
        
    
end



if write_animations
    imwrite(imall,map,'spectral_spatial_RF_animation.gif','DelayTime',0,'LoopCount',Inf) %g443800
end

return
%%
figure
subplot(421)
plot([0:Nspec-1]*DT*1e3, real(rfspec), '-x')
ylabel('RF')
title('Spectral Pulse Design')
subplot(423)
plot([0:Nspec-1]*DT*1e3, ones(1,Nspec))
ylabel('G_f')
xlabel('Time (ms)')
subplot(222)
plot(f, abs(mxy_f))
xlabel('Frequency (Hz)'), ylabel('M_{XY}')
title('Spectral Profile (z=0)')

subplot(425)
plot([0:Nlobe-1]*dt*1e3, real(rfspatv))
ylabel('RF')
title('Spatial Pulse Design')

subplot(427)
plot([0:Nlobe-1]*dt*1e3, gamp*g1)
ylabel('G_z')
xlabel('Time (ms)')
subplot(224)
plot(z, abs(mxy_z))
xlabel('Position (cm)'), ylabel('M_{XY}')
title('Spatial Profile (f=0)')


figure
subplot(421)
plot([0:length(rf)-1]*dt*1e3, real(rf), ...
    [0.5:1:Nspec-0.5]*DT*1e3, real(rfspec)/max(real(rfspec)) * max(real(rf)), '--')
ylabel('RF')
title('Spectral-Spatial RF Pulse')
subplot(423)
plot([0:length(rf)-1]*dt*1e3, g)
ylabel('G_z')
xlabel('Time (ms)')

subplot(222)
plot([0:length(rf)-1]*dt*1e3, GAMMA*dt*(cumsum(g) - sum(g)))
xlabel('k_f = t'), ylabel('k_z')
title('k-space trajectory')

subplot(223)
plot3([0:length(rf)-1]*dt*1e3, GAMMA*dt*(cumsum(g) - sum(g)), real(rf))
xlabel('k_f = t'), ylabel('k_z')
title('B_1(k)')

subplot(224)
imagesc(f,z,abs(mxy_2d))
xlabel('Frequency (Hz)'), ylabel('Position (cm)')
title('Spectral-spatial Profile')
colormap(gray)

return
%% true-null design

rfspec = rfspec .* exp(i*pi*[1:length(rfspec)]);

rf = []; g = [];
for k = 1:Nspec
   rf = [rf, rfspatv*rfspec(k)];
   g = [g, (mod(k,2)*2 - 1)*gamp*g1];
end

g = [g, (mod(Nspec+1,2)*2-1)*gamp*grw];
rf = [rf, zeros(1,Nrw)];

rf = flip_scale*rf;
rfs = rfscaleg(rf, T);

%%

mxy_z = ab2ex(abr(rf, gz, z));
mxy_f = ab2ex(abr(rf, gf, f));
mxy_2d = ab2ex(abr(rf, gz + i*gf, z, f));


%%
figure
subplot(421)
plot([0:Nspec-1]*DT*1e3, real(rfspec), '-x')
ylabel('RF')
title('Spectral Pulse Design')
subplot(423)
plot([0:Nspec-1]*DT*1e3, ones(1,Nspec))
ylabel('G_f')
xlabel('Time (ms)')
subplot(222)
plot(f, abs(mxy_f))
xlabel('Frequency (Hz)'), ylabel('M_{XY}')
title('Spectral Profile (z=0)')

subplot(425)
plot([0:Nlobe-1]*dt*1e3, real(rfspatv))
ylabel('RF')
title('Spatial Pulse Design')

subplot(427)
plot([0:Nlobe-1]*dt*1e3, gamp*g1)
ylabel('G_z')
xlabel('Time (ms)')
subplot(224)
plot(z, abs(mxy_z))
xlabel('Position (cm)'), ylabel('M_{XY}')
title('Spatial Profile (f=0)')


figure
subplot(421)
plot([0:length(rf)-1]*dt*1e3, real(rf), ...
    [0.5:1:Nspec-0.5]*DT*1e3, real(rfspec)/max(real(rfspec)) * max(real(rf)), '--')
ylabel('RF')
title('Spectral-Spatial RF Pulse')
subplot(423)
plot([0:length(rf)-1]*dt*1e3, g)
ylabel('G_z')
xlabel('Time (ms)')

subplot(222)
plot([0:length(rf)-1]*dt*1e3, GAMMA*dt*(cumsum(g) - sum(g)))
xlabel('k_f = t'), ylabel('k_z')
title('k-space trajectory')

subplot(223)
plot3([0:length(rf)-1]*dt*1e3, GAMMA*dt*(cumsum(g) - sum(g)), real(rf))
xlabel('k_f = t'), ylabel('k_z')
title('B_1(k)')

subplot(224)
imagesc(f,z,abs(mxy_2d))
xlabel('Frequency (Hz)'), ylabel('Position (cm)')
title('Spectral-spatial Profile')
colormap(gray)
